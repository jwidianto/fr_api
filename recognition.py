            ...
            if to_mysql: 
                with DBSESSION as connection :
                    connection.execute(
                        text(
                            """
                                INSERT INTO fail_image (`image_location`, `exception`) 
                                VALUES ('"""+str(employee)+"""', '"""+str(err)+"""')
                            """
                        )
                    )
                    connection.commit()
...
def to_pickle(
    ...
    result = DBSESSION.execute(text("""SELECT DISTINCT `DEPT` FROM representations WHERE `DEPT` IS NOT NULL"""))
    DBSESSION.close()
    for row in result:
        departemen = row[0] 
        ...
        db_representations = DBSESSION.execute(text("""SELECT `image_location`, `pickle_data` FROM representations WHERE `DEPT` = '"""+departemen+"""'"""))
        DBSESSION.close()
        for rep in db_representations :
            pickle_data = json.loads(rep[1])
            if not silent:
    ...
    result = DBSESSION.execute(text("""SELECT DISTINCT `GOLONGAN` FROM representations WHERE `GOLONGAN` IS NOT NULL"""))
    DBSESSION.close()
    for row in result:
        golongan = row[0] 
        ...
        db_representations = DBSESSION.execute(text("""SELECT `image_location`, `pickle_data` FROM representations WHERE `GOLONGAN` = '"""+golongan+"""'"""))
        DBSESSION.close()
        for rep in db_representations :
            pickle_data = json.loads(rep[1])
            if not silent:
